...
+function ($) {
	...
	var months = ['January','February','March','April','May','June','July','August','September','October','November','December'],
		...
		DCalendar = function(elem, options) {
			...
			/* Code Changes*/
			var check_date_today = new Date();
			var __getFullYear = check_date_today.getFullYear() - 16;
			var __month = check_date_today.getMonth() + 1;//Note 0=> january
			var __date = check_date_today.getDate();
					//calendar container
			this.today = new Date(__getFullYear + '-' + __month + '-' + __date);	//current date
			/**/
			//current selected date, default is today if no value given
			this.date = this.elem.val() === '' ? new Date(__getFullYear + '-' + __month + '-' + __date) : this.reformatDate(this.elem.val()).date;
			this.viewMode = 'days';
